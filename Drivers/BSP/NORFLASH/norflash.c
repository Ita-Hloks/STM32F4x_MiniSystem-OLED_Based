...
uint16_t g_norflash_type = GD25Q40;     /* 默认是GD25Q40 */
...
void norflash_init(void)
{
    ...
    spi1_init();                          /* 初始化SPI1 */
    spi1_set_speed(SPI_SPEED_2);          /* SPI1 切换到高速状态 30Mhz */
...
{
    NORFLASH_CS(0);
    spi1_read_write_byte(FLASH_WriteDisable);  /* 发送写禁止指令 */
    NORFLASH_CS(1);
}
...
void norflash_send_address(uint32_t address)
{
    ...
    {
        /* GD25Q40E和其他芯片使用24位地址（3字节） */
        address &= 0x00FFFFFF;  /* 确保地址不超过24位 */
    }
...
uint16_t norflash_read_id(void)
{
    ...
    NORFLASH_CS(0);
    spi1_read_write_byte(FLASH_ManufactDeviceID);   /* 发送读ID 命令 */
...
void norflash_erase_chip(void)
{
    ...
    NORFLASH_CS(0);
    spi1_read_write_byte(FLASH_ChipErase);  /* 发送全片擦除命令 */ 
    NORFLASH_CS(1);
